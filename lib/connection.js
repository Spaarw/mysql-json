...
module.exports = function(host) {
     ...
     * @param callback
     */
    _self.findById = function(tableName, id, callback) {
    	if (id > 0) {
        	var primaryKeyId = 'id';
       		var primaryKeyQuery = 'SHOW KEYS FROM `' + tableName + '` WHERE Key_name = \'PRIMARY\'';
           	_self.query(primaryKeyQuery, function (err, res) {
                	if (err) {
                    		callback(new Error('mysql-json [find]: Cannot detect primary key field'), null);
                	}
			else {
                    		primaryKeyId =  res[0]['Column_name'];
		    		var mysqlQuery = 'SELECT * FROM `' + tableName + '` WHERE ' + primaryKeyId + '=' + id;
                    		_self.query(mysqlQuery, callback);
                	}
            	});
        }
        else {
        	callback(new Error('mysql-json [find]: id missing or invalid'), null);
        }
    };
